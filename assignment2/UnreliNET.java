// (C) 2015, 2016, 2017 Marc Bruenink
// (C) 2014 Hong Hande
...
public class UnreliNET {
    ...
    public class Forwarder extends Thread {
        ...
        public void run() {
            try {
                ...
                //System.out.println("Starting Forwarder to " + this.dst_addr + ":" + this.dst_port 
                //                + " with corruption rate of " + this.corrupt_rate + " and drop rate of " + this.drop_rate);
                while (true) {
    ...
    public UnreliNET(float data_corrupt_rate, float ack_corrupt_rate,
                     float data_loss_rate, float ack_loss_rate, 
                     int min_propagation_delay, int max_propagation_delay,
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 9) {
            System.err.println("Usage: java UnreliNET <P_DATA_CORRUPT> <P_ACK_CORRUPT> " +
                               "<P_DATA_LOSS> <P_ACK_LOSS> <MIN_PROPAGATION_DELAY> <MAX_PROPAGATION_DELAY> <unreliNetPort> <rcvHostName> <rcvPort>");
            System.exit(-1);
        } else {
